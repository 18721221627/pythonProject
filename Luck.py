...
#要识别的验证码--就是那个小块！
yzm_btn = browser.find_element_by_xpath('//*[@id="signupForm"]/div[1]/div[2]/a/img')
# 获取图片元素的位置
...
re = chaojiying.PostPic(im, 6001)
print('我是返回数据', re)
#返回的数据是个json，pic_str这个就是验证码识别的结果！
distance = re['pic_str']

print('验证码是',distance)
#输入账号
browser.find_element_by_name('username').send_keys('caoyu')
#输入密码
browser.find_element_by_name('password').send_keys('caoyu')
# 在输入框输入验证码
browser.find_element_by_xpath('//*[@id="signupForm"]/div[1]/div[1]/input').send_keys(distance)
time.sleep(3)
#点击登录
